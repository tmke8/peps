...
Attribute chains after a leading dot (``.ns.CONST``) are not permitted; see
`Rejected Ideas`_. 
...
namespace and ``.RED`` when it does not.
...
case distinction (e.g. CJK) could never be matched as values.
